...
class _EditTransactionScreenState extends State<EditTransactionScreen> {
  ...
  void initState() {
    ...
    // Get the transaction passed as argument
    ...
    _amountController.text =
        CurrencyUtils.formatCurrency(transaction.amount.abs());
    ...
    _originalCategoryId = transaction.category;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
            _type == TransactionType.expense ? 'Edit Expense' : 'Edit Income'),
      ),
      body: Obx(() {
  ...
  void _updateTransaction() async {
    if (_formKey.currentState!.validate()) {
      ...
      double amount = CurrencyUtils.parseAmount(_amountController.text);
      ...
      final updatedTransaction = Transaction(
        id: _originalTransaction.id, // Keep the original ID
        amount: amount,
        type: _type,
        date: _selectedDate,
      ...
      await _transactionController.updateTransaction(updatedTransaction);

      // Handle budget updates for expenses
      if (_originalTransaction.isExpense || updatedTransaction.isExpense) {
        // Remove original expense amount from original category
        if (_originalTransaction.isExpense) {
          await _budgetController.updateBudgetSpentAmount(
            _originalCategoryId,
            _originalAmount, // Original amount is already negative for expenses
          );
        }
        ...
        if (updatedTransaction.isExpense) {
          await _budgetController.updateBudgetSpentAmount(
            _selectedCategoryId,
            amount, // Updated amount is negative for expenses
          );
        }
      }
